...
class TkinterApp:
    ...
    def __init__(
        self, appname: str = "TkinterApp", width: int = 400, heigth: int = 400
    ):
        self.root = tk.Tk()
        ...
        self.append_button = tk.Button(
            self.root, text="Append random values", command=self.append_random
        )
    ...
    def create_buttons_labels(self, column: str):
        self.buttons[column] = tk.Button(
            self.root,
            text=f"Get column {column}",
            command=lambda: self.get_column(column),
        )
