...
import os
from tkinter.constants import BOTH, BOTTOM, HORIZONTAL, LEFT, NONE, NS, NSEW, NW, RIGHT, VERTICAL, X, Y
...
class TkinterApp:
    def __init__(self, appname="TkinterApp", width=400, height=400):
        ...
        if os.name == 'nt':
            self.work_frame.bind('<MouseWheel>', lambda event: self.canvas.yview('scroll', -1 if event.num==4 else 1, 'units'))
        else:
            self.work_frame.bind('<Button-4>', lambda event: self.canvas.yview('scroll', -1 if event.num==4 else 1, 'units'))
            self.work_frame.bind('<Button-5>', lambda event: self.canvas.yview('scroll', -1 if event.num==4 else 1, 'units'))
        
        self.canvas.create_window((0,0), window=self.work_frame, anchor=NW)
        
        self.vertical_scroll = tk.Scrollbar(self.canvas, orient=VERTICAL, command=self.canvas.yview)
        self.canvas.config(yscrollcommand=self.vertical_scroll.set)
        self.vertical_scroll.pack(side=RIGHT, fill=Y)
        
        self.horizontal_scroll = tk.Scrollbar(self.canvas, orient=HORIZONTAL, command=self.canvas.xview)
        ...
        self.labellink = HTMLLabel(self.work_frame, html='<a href="https://www.youtube.com">YouTube</a>', height=3)
        ...
        self.label_img_local = HTMLLabel(self.work_frame, html='<img src="images/0.jpg" />')
        ...
        self.canvas_right.bind('<Configure>', lambda event: self.canvas_right.configure(scrollregion=self.canvas_right.bbox('all')))
        
        self.work_frame_right = tk.Frame(self.canvas_right, background="red")
        
        if os.name == 'nt':
            self.work_frame_right.bind('<MouseWheel>', lambda event: self.canvas_right.yview('scroll', -1 if event.num==4 else 1, 'units'))
        else:
            self.work_frame_right.bind('<Button-4>', lambda event: self.canvas_right.yview('scroll', -1 if event.num==4 else 1, 'units'))
            self.work_frame_right.bind('<Button-5>', lambda event: self.canvas_right.yview('scroll', -1 if event.num==4 else 1, 'units'))
        
        self.canvas_right.create_window((0,0), window=self.work_frame_right, anchor=NW)
        
        self.vertical_scroll_right = tk.Scrollbar(self.canvas_right, orient=VERTICAL, command=self.canvas_right.yview)
        self.canvas_right.config(yscrollcommand=self.vertical_scroll_right.set)
        self.vertical_scroll_right.pack(side=RIGHT, fill=Y)
        
        self.horizontal_scroll_right = tk.Scrollbar(self.canvas_right, orient=HORIZONTAL, command=self.canvas_right.xview)
        ...
        self.labelhtml_right = HTMLLabel(self.work_frame_right, html='<h1>Hello World</h1>\
