...
class TkinterApp:
    def __init__(self, appname="TkinterApp", width=400, height=400):
        ...
        self.textbox = tk.Text(self.frame, width=40, height=10, font=self.text_font)
        ...
        self.listbox_sizes = tk.Listbox(self.fonts_frame, selectmode=SINGLE, width=5)
        ...
        self.listbox_weights = tk.Listbox(self.fonts_frame, selectmode=SINGLE, width=10)
        ...
        self.listbox_slants = tk.Listbox(self.fonts_frame, selectmode=SINGLE, width=10)
        ...
        self.listbox_overstrike.bind("<ButtonRelease-1>", self.change_overstrike)

    def change_family(self, event: Event):
        self.text_font.config(
            family=self.listbox_families.get(self.listbox_families.curselection())
        )
        return "break"

    def change_size(self, event: Event):
        self.text_font.config(
            size=self.listbox_sizes.get(self.listbox_sizes.curselection())
        )
        return "break"

    def change_weight(self, event: Event):
        self.text_font.config(
            weight=self.listbox_weights.get(self.listbox_weights.curselection())
        )
        return "break"

    def change_slants(self, event: Event):
        self.text_font.config(
            slant=self.listbox_slants.get(self.listbox_slants.curselection())
        )
        return "break"

    def change_underline(self, event: Event):
        self.text_font.config(
            underline=self.listbox_underline.get(self.listbox_underline.curselection())
        )
        return "break"

    def change_overstrike(self, event: Event):
        self.text_font.config(
            overstrike=self.listbox_overstrike.get(
