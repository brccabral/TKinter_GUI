...
class TkinterApp:
    def __init__(self, appname="TkinterApp", width=400, height=400):
        ...
        self.button = tk.Button(
            ...
            command=lambda: self.open_web_browser(None),
        )
        ...
        self.button = tk.Button(
            self.root, text="Open Firefox", command=lambda: self.open_firefox(None)
        )
        self.button.pack(padx=5, pady=5)
        self.button.config(cursor="fleur")
    ...
    def open_firefox(self, event: tk.Event):
        if os.name == "nt":
            webbrowser.get("C:/Program Files/Mozilla Firefox/firefox.exe %s").open_new(
                "http://github.com"
            )
        else:
